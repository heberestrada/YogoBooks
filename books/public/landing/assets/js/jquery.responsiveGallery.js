...
;(function($){
    $.fn.responsiveGallery = function(option){
		var opts = $.extend({}, $.fn.responsiveGallery.defaults, option), //配置选项
			...
			rgShowCount = 5,
            rgCurrentIndex =  opts.currentIndex - Math.floor(rgShowCount / 2),
            rgCurrentIndex = rgCurrentIndex > 0 ? rgCurrentIndex : rgItemsLength + rgCurrentIndex,
			rgTansCSS = [],
		...
		function getTransform1CSS(){
			...
			if(support3d){
				...
				cssArray = [
					...
					getTransform2dCSS('0', 	1,		'50%',		'100%',	1,	'visible'),
        ...
        function selectIndex(index){
            var newIndex = index - Math.floor(rgShowCount / 2);
            newIndex = newIndex > 0 ? newIndex : rgItemsLength + newIndex;
            var dif = newIndex - rgCurrentIndex
		...
		$(window).on('resize', function(e){
			...
			//rgCurrentIndex = Math.round(rgShowCount / 2) + opts.currentIndex;
			moveGallery(0);
			setTimeout(function(){
